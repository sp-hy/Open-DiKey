...
class DiKeyProbeActivity : AppCompatActivity() {
    ...
    private lateinit var client: DiKeyBleClient
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        client = DiKeyBleClient(
            context = this,
            onStatus = { msg ->
                connStatusText.text = msg
                if (msg.startsWith("TX ·") || msg.startsWith("Write") || msg.startsWith("Restoring")) {
                    appendLogLine(msg)
                }
                when {
    ...
    override fun onStart() {
        super.onStart()
        if (!client.isConnected && !autoConnectInFlight) {
            autoConnectArmed = true
            startAutoScan()
        }
    }
    ...
    private fun startAutoScan(clearFirst: Boolean = false) {
        if (client.isConnected || autoConnectInFlight) return
    ...
    private fun maybeAutoConnect(devices: List<ScannedBleDevice>) {
        if (!autoConnectArmed || autoConnectInFlight || client.isConnected) return
    ...
    private fun applyDial() {
        ...
        val ok = client.sendDialDisplay(left, type.code, value)
        Snackbar.make(
            connStatusText,
